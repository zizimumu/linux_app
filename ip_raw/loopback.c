...
void  handle_receive(void )
{
 ...
 void raw_socket_recv(char *dev)
 {
	...
	if( ret ){
     ...
     while (1)
     {
         ...
         ret_len = recv(sock_raw_fd, ether_frame, 4096, 0);
 ...
 int main(int argc, const char *argv[])
 {
    ...
    if(argc < 3){
        printf("parameters error, use as : %s [ethernet device] [size_MB], e.g. send eth0 100\n",argv[0]);
        return 0;
    }
    ...
    if(pid < 0){
        ...
        raw_send(argv[1],size*1024*1024);

    }
    else{


        printf("receiver processer: pid %d\n",pid);
