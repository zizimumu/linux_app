...
#define ETH_P_DEAN 0x0855 //自定义的以太网协议type
 ...
 void raw_send(char *dev, unsigned int size)
 {
    int sd,ret;
    uint8_t src_mac[6];
    uint8_t dst_mac[6];;    
    ...
    struct ifreq ifr;
    char *interface=dev;
    unsigned int len = 0,left =0,frame_len;
    ...
    memset(ether_frame + 14,0xaa,sizeof(ether_frame) - 14);
    ...
    while(1){
        // 14 + strlen(data)
        left = (size - len);
        frame_len = left < MAX_MTU ? left : MAX_MTU;
        ret = sendto(sd, (unsigned char *)ether_frame,frame_len+14, 0, (const struct sockaddr *)&stTagAddr, sizeof(stTagAddr));
        if(ret <=0){
 ...
 int main(int argc, const char *argv[])
 {
    ...
    //raw_send(argv[1]);
    if(argc < 3){
        printf("parameters error, use like: send eth0 send_data_size\n");
        return 0;
    }
    
    size    = (unsigned int)strtoul(argv[2], NULL, 0);
     //if(size <= 14){
     //    printf("wrong size, need >= 14\n");
     //    return ;
     //}
    raw_send(argv[1],size);
     return 0;
 }