...
#define ETH_P_DEAN ETH_P_ALL   //  0x0855 //自定义的以太网协议type
...
void rev_sig_handle(int signo)
{
 ...
 void raw_send(char *dev,char *mac_addr)
 {
    int sd,ret,mtu,i;
    uint8_t src_mac[6];
    uint8_t dst_mac[6];;    
    ...
    while(1){
        ...
        frame_len =  mtu+14;
        ret = sendto(sd, (unsigned char *)ether_frame,MAX_PACKETS*frame_len, 0, (const struct sockaddr *)&stTagAddr, sizeof(stTagAddr));
        if(ret <=0){
 ...
 int main(int argc, const char *argv[])
 {
    ...
    if(argc < 2){
        printf("parameters error, use like: %s eth0\n",argv[0]);
        return 0;
    }
