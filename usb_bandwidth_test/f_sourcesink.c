...
static struct usb_endpoint_descriptor fs_source_desc = {
	...
	.bEndpointAddress =	USB_DIR_OUT, //USB_DIR_IN,
...
static void source_sink_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	case 0:				/* normal completion? */
		//if (ep == ss->out_ep) {
			...
			//printk("Bulk receive %d bytes done\n",req->actual);

			if(counter == 0)
				timer_start = ktime_get();
				
			counter += req->actual;
			if(counter >= 400*1024*1024){
				counter = 0;
				timer_end = ktime_get();
				...
				printk("400MB write done\n");

			}
		//}
		//else if(ep == ss->iso_out_ep){

			//printk("ISO receive %d bytes done\n",req->actual);
		//};
		break;
...
static ssize_t f_ss_opts_counter_show(struct config_item *item, char *page)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int result;
...
static ssize_t f_ss_opts_counter_store(struct config_item *item,
					   const char *page, size_t len)
{
	struct f_ss_opts *opts = to_f_ss_opts(item);
	int ret;
	...
	printk("clearing counter\n");
	counter = 0;
	return ret;
}

CONFIGFS_ATTR(f_ss_opts_, counter);
