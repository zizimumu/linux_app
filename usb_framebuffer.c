...
static unsigned char in_dex[256];
...
#define TARGET_TEST_BYTES (100*1024*1024)
#define URB_QUEUE_LEN 2
			 ...
			 loff_t *ppos)
{
	...
	print_hex(in_dex,sizeof(in_dex));
#if 0
	dev = file->private_data;
...
static void skel_write_callback(struct urb *urb)
{
	...
	//printk("urb cb\n");
	g_count += urb->actual_length;
	if(g_count >= TARGET_TEST_BYTES){

		//timer_end = ktime_get();
		
		g_count = 0;

		//speed  = TARGET_TEST_BYTES*1000 / (timer_end.tv64/1000000 - timer_start.tv64/1000000);
		//frames = speed / dev->dma_len;
		//printk("usb speed %B/s, frame speed %fps\n",speed,frames);

		//timer_start.tv64 = timer_end.tv64;
		//printk("start time : %lld ns\n",timer_start.tv64);
		//printk("end time   : %lld ns\n",timer_end.tv64);
	
		//printk("write done\n");
		// printk("send 100MB data\n");
	}
...
static void skel_prepare_urbs(struct usb_skel *dev,struct urb		*urbs[],__u8 endpoint )
{
	...
	int i;
	unsigned int len,sect_sz,offset,tmp,size,free;
	unsigned long start ;
	
	len = dev->dma_len;
	start = dev->dma_addr;

	sect_sz = len / URB_QUEUE_LEN;

	sect_sz = sect_sz + (4096 - sect_sz % 4096);
	

	printk("sector size %d KB, queue number %d\n",sect_sz,(len + sect_sz -1)/sect_sz);
	printk("dma virtual addr 0x%x, dma phy addr 0x%x\n",(unsigned long)dev->fbmem_virt,(unsigned long)dev->dma_addr);
	...
	for(i=0;;i++){
		...
		//}
		//memset(buf,0x55,URB_BUF_SIZE);
		buf = dev->fbmem_virt + offset;
		urb->transfer_dma = start + offset;
		free = len - offset;
		...
		usb_fill_bulk_urb(urb, dev->udev,
				  ...
				  buf, size, skel_write_callback, dev);
		urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;

		urbs[i] = urb;
		
		printk("fill urp, buf[0] 0x%x, size %d\n",buf[0],size);
...
static void skel_prepare_same_urbs(struct usb_skel *dev,struct urb		*urbs[],__u8 endpoint )
{
...
void fill_buff(unsigned char *buf, int len)
{
	unsigned int i,sect_sz;
	unsigned char val = 0;
		      ...
		      const struct usb_device_id *id)
{
	...
	info = framebuffer_alloc(sizeof(u32) * 256, &interface->dev);
	if (!info)
		goto error;
	...
	fbmem_virt = dma_alloc_coherent(NULL,
					...
					GFP_KERNEL);
	if (!fbmem_virt) {
		dev_err(&interface->dev, "unable to allocate  fb memory\n");
		goto error;
	}
	...
	dev->fbmem_virt = fbmem_virt;
	dev->dma_addr = info->fix.smem_start;
	dev->dma_len = info->fix.smem_len;

	retval = fb_alloc_cmap(&info->cmap, 256, 0);
	if (retval < 0){
		dev_err(&interface->dev, "unable to allocate  fb cmap\n");
		goto error;
	}
	...
	if (retval < 0){
		dev_err(&interface->dev, "unable to register framebuffer\n");
		goto error;
	}

	dev_info(&interface->dev, "fb%d: %s frame buffer device at 0x%x+0x%x\n",
	...
	if(dev->bulk_out_endpointAddr){
		...
		fill_buff(dev->fbmem_virt,dev->dma_len);

		// the USB host may schedule the URB in a no-sequency way even set to two queue. e.g. 1,1,1,2,1,2
		// so we need to use skel_prepare_same_urbs
		// skel_prepare_urbs(dev,dev->bulk_out_urb, dev->bulk_out_endpointAddr);
		skel_prepare_same_urbs(dev,dev->bulk_out_urb, dev->bulk_out_endpointAddr);
		skel_send_urbs(dev,dev->bulk_out_urb);
		printk("usb data started\n");
	}
...
static void skel_disconnect(struct usb_interface *interface)
{
	...
	dma_free_coherent(NULL, dev->fb_info->fix.smem_len, dev->fb_info->screen_base,
			  dev->fb_info->fix.smem_start);
